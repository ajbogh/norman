...
function bindListeners(micInputStream, micInstance, outputFileStream){
  ...
  micInputStream.on('data', function(data) {
    ...
    for(i=0; i<chunk.length; i=i+2) {
      ...
      if(Math.abs(speechSample) > 2000) {
        ...
        // if (debug) {
        console.log("Found speech block", speechData, typeof speechSample);
        // }
    ...
    if(silenceLength == chunk.length / 2) {
      ...
      if(consecutiveSpeechBlocks > 1 && consecutiveSilence === numSilenceFramesExitThresh) {
        // self.emit('silence');
  ...
  micInputStream.on('silence', function() {
    ...
    console.log(consecutiveSpeechBlocks);
